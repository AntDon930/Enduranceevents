...
| `cyclingaustria.at/kalender` (Österreichischer Radsport-Verband, 72 Rennen bis 09/2027) | robots.txt Joomla-Voreinstellung (nicht `/index.php`, nicht `/kalender`), Impressum nur Bildnachweis, keine AGB | **`cyclingaustria_scraper.py`** (24.09.2026): Listenseiten `?view=events&page=N` → Rennseite mit Start/Ziel (PLZ + Ort), Datum, Sparte, Disziplin, Blöcken „Rennen N" (Klassen, Distanz oder Renndauer) und „zur Website" (Veranstalter). Alle Klassen, auch Lizenz (Entscheidung des Nutzers); nicht übernommen: Kunstrad/Radball, Downhill/Enduro/E-MTB/Technikparcours, Madison und Mannschaftszeitfahren, Nachwuchs (Klassen bis U17 oder „Kids/Youngster/Nachwuchs" im Namen). Ein Cyclocross ohne Kilometer ist ein Zeitrennen („Cyclocross 40 min", `dauer_h`). Erster Lauf: 62 neue Events, 62 mit Veranstalterseite |
| `swimsports.ch/schwimmkalender` (Verband der Schweizer Schwimmschulen, Freiwasser CH) | robots.txt Drupal-Voreinstellung, Impressum/Datenschutz ohne Verbot; **die Adresse ohne `/de/`** – mit Sprachpräfix antwortet die Seite „Zugriff verweigert" (kein Bot-Schutz, falscher Pfad) | **`swimsports_scraper.py`** (24.09.2026): Liste (`article.swimming-calendar`, Datum, Ort) → `/node/<nr>` mit Distanz („2 km", „1.8 Km", „Schwimmen 1.1 km/SUP Fun 1.5 km/…" – SUP zählt nicht, „Libre" = ohne Maß), Gewässer, Homepage. Absagen fallen (außer der Name nennt den nächsten Termin: Rhyschwümme 08.08.2027). Im September 2026 nur 2 künftige Anlässe (33 gelistet, fast alle 2026) – die Saison 2027 kommt im Frühjahr |
| `fsieben.at/tools/triathlon-kalender-oesterreich/` (F7 Ausdauertraining, Rechercheliste „Stand 29.07.2026") | robots.txt WordPress (nur `/wp-admin/`), Impressum mit üblichem Urheberrechtshinweis, kein Verbot | **`fsieben_scraper.py`** (24.09.2026): JSON-Array in `<pre id="f7-ev-data">` (Name, ISO-Datum, Ort, Bundesland, Formate, Event-Seite, Status). **52 von 94 Bewerben sind „Termin folgt" ohne Datum** – die kommen nicht (Datenregel 19); 19 datierte künftige, 15 davon schon im Bestand. Je Format eine Zeile ohne Kilometer (Sprint/Olympisch/Mitteldistanz/Langdistanz, Cross, Duathlon, Aquathlon), „unbestätigt" = vorläufig, Ort über `places.json` je Bundesland („Pichlingersee, Linz" → Linz). Erster Lauf: 13 neue Events |
| `triathlon-austria.at/de/service-termine` (ÖTRV) | robots.txt frei, Impressum ohne Verbot | **kein Scraper**: nur 4 künftige Termine in der Liste (alle bei fsieben bzw. im Bestand); fsieben verlinkt die Seite bei Veranstaltungen ohne eigene Seite → `PORTAL_DOMAINS` |
| `seeueberquerung.ch` (Stadtzürcher Seeüberquerung) | robots.txt frei | **kein Scraper**, ein Anlass: 07.07.2027, 1'500 m Mythenquai → Tiefenbrunnen, „kein Wettkampf", 9'000 Plätze – in `manual_events.json`; swimsports.ch führt erst den Termin 2026 |
...
### Die Linkliste des Nutzers (24.09.2026): 125 Adressen, 91 Domains

Nach dem PDF hat der Nutzer eine Liste von 125 Adressen geschickt (Rad-,
Schwimm- und Triathlon-Kalender in DE/AT/CH/Südtirol) – „Bitte alle
checken". Für jede Domain wurden robots.txt, Impressum/AGB und die Seite
selbst abgerufen (Wegwerf-Skript, Ergebnis nicht im Repo; die Tabelle oben
trägt die Quellen mit Scraper). Bilanz:
...
Zwei Dinge daraus: **Die österreichischen Triathlons stehen zu einem
Großteil ohne Termin da** – fsieben nennt für 52 Bewerbe nur „Termin
folgt"; sie kommen, sobald die Veranstalter 2027 ausschreiben (der
wöchentliche Lauf holt sie). Und **Freiwasserschwimmen ist eine
